...
function MKUFormatter() {
  ...
  const formatStudentData = (rawData) => {
    ...
    return {
      ...
      motherName: data[2]?.trim() || "Not mentioned",
      course: data[12]?.trim() || "Not mentioned",
      branch: data[13]?.trim() || "Not mentioned",
      semester: semester,
      aadharNumber: data[7]?.trim() || "Not mentioned",
      city: `${data[8]?.trim() || ""}`.trim(),
      state: getStateFromString(rawData), // Use the state-matching function
      studentContact: `${data[3]?.trim() || ""}, ${data[4]?.trim() || ""}`.trim(),
      email: data[5]?.trim() || "Not mentioned",
    };
  };
  ...
  // Handler to copy the student names to clipboard
  const handleCopyNames = () => {
    if (formattedData) {
      const names = `${formattedData.studentName} ${formattedData.fatherName} ${formattedData.motherName}`;
      navigator.clipboard.writeText(names);
      alert("Student names copied to clipboard!");
    }
  };
